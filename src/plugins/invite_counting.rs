        ...
        match event {
            ...
            Event::MemberAdd(e) => {
                ...
                if !oauth {
                    ...
                    let cache: Vec<MongoInvite> =
                        storage.invites.into_iter().map(Into::into).collect();
                        ...
                        if let Some(user) = &invite.inviter {
                                ...
                                .any(|i| i == &member.user.id.get().to_string())
                            {
                                user_coll.find_one_and_update(
                                    ...
                                    doc! { "$inc": { "leaves": 1 }, "$push": { "leaves_data": bson::to_bson(&invite).unwrap() }, "$addToSet": { "regular_data": bson::to_bson(&invite).unwrap() } },
                                    None,
                                ).await?;
                            ...
                            join_coll.find_one_and_update(
                                doc! {"doctype": "join_storage", "guild_id": member.guild_id.0.to_string(), "user_id": member.user.id.get().to_string()},
                                doc! { "$set": { "inviter_id": Some(user.id.get().to_string()) } },
                                None,
                            ).await?;
                        } else {
            ...
            Event::MemberRemove(event) => {
                ...
                let user = coll.find_one(doc!{ "user_id":event.user.id.get().to_string(), "guild_id":event.guild_id.get().to_string(), "doctype":"join_storage" }, None).await?;

                if user.is_some() {
                    let invite_coll = ctx.db.collection::<UserInviteStorage>("invites");
                    invite_coll.find_one_and_update(
                        doc! { "user_id":event.user.id.get().to_string(), "guild_id":event.guild_id.get().to_string() },
                        doc! { "$inc": { "leaves": 1 }, "$addToSet": { "leaves_data": event.user.id.get().to_string() } },
                        None,
                    ).await?;
                }
            }
